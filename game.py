...
# TPV — ЭТАП 11.5: ROOM / SCREEN SERVICE
from tpv.services import create_tpv_room_service

TPV_ROOM_SERVICE = create_tpv_room_service(
    socketio,
    default_room_code=DEFAULT_ROOM_CODE,
    get_current_room=get_current_room,
)

# Совместимые имена для существующих маршрутов и Socket.IO.
get_room_code = TPV_ROOM_SERVICE.get_room_code
emit_tpv_host = TPV_ROOM_SERVICE.emit_host
emit_tpv_spectator = TPV_ROOM_SERVICE.emit_spectator
emit_tpv_players = TPV_ROOM_SERVICE.emit_players
emit_tpv_player = TPV_ROOM_SERVICE.emit_player

# КОНЕЦ ROOM / SCREEN SERVICE
...
# TPV — ЭТАП 11.6: MEDIA ROUTES
from tpv.media_routes import register_tpv_media_routes

TPV_MEDIA_ROUTE_EXPORTS = register_tpv_media_routes(
    app,
    number_to_audio=number_to_audio,
)
serve_audio_tpv = TPV_MEDIA_ROUTE_EXPORTS["serve_audio_tpv"]
serve_audio_tpv_bong = TPV_MEDIA_ROUTE_EXPORTS["serve_audio_tpv_bong"]
api_voice_number = TPV_MEDIA_ROUTE_EXPORTS["api_voice_number"]

# КОНЕЦ MEDIA ROUTES


# TPV — ЭТАП 11.6: UNIFIED APPLICATION BOOTSTRAP
from tpv.application import register_tpv_application

TPV_APPLICATION_EXPORTS = register_tpv_application(
    globals(),
